...
##  This script processes 2211KMI-0002                   ##
##                                                       ##
##  Mangrove fungi microbiome                            ##
##                                                       ##
...
plotQualityProfile(fnFs[c(10,45)]) # fwd reads # drops below Q30 at around 270
plotQualityProfile(fnRs[c(10,150)]) # rev reads # drops below Q30 at around 200
...
system2(cutadapt, args = "--version") # running in windows
...
plotQualityProfile(filtFs[c(20,30)]) # maybe too strict? 
plotQualityProfile(filtRs[c(20,30)]) # looks ok
...
filtFs <- list.files("./2211KMI-0002/filtered", pattern = "_F_filt", full.names = TRUE)
filtRs <- list.files("./2211KMI-0002/filtered", pattern = "_R_filt", full.names = TRUE)
...
taxa <- assignTaxonomy(seqtab.nochim,"./sh_general_release_dynamic_29.11.2022.fasta", minBoot = 80,multithread = TRUE)
saveRDS(taxa,"./Output/taxa.RDS")

taxa <- addSpecies(taxa, "./sh_general_release_dynamic_29.11.2022.fasta")
...
blanks = which(ps@sam_data$Site == 'NA')
...
# Export OTU table as text file (Samples as columns, ASVs as rows): o produce match list with blastn
