...
assignments <- data.frame(Kingdowm = king, Phylum=phy, Class=cla,Order=ord,Family=fam,Genus=gen,Species=spp)

assignments %>% pivot_longer(1:6) %>% mutate(name=factor(name,levels = c("Kingdom", "Phylum","Class","Order","Family","Genus","Species"))) %>%
...
rm(king,phy,cla,ord,fam,gen,spp,assignments,ps_sp)
...
# Rarefaction curves
png("./Plots/Overall_Rarefaction_Curve.png")
